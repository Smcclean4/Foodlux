...
const Homeitems = ({ menu, title, addtocart, searchinfo, render, setrender }) => {
  ...
  // filter of all items that are food items 
  const filteredFoodItems = menu.filter((val: { type: string; }) => val.type === 'food')
  // filter of all items that are drink items
  const filteredDrinkItems = menu.filter((val: { type: string; }) => val.type === 'drink')
      ...
      {!render ? (
        ...
        >
          ...
          <Grid
            ...
            {filteredFoodItems?.map((items: { type: string; image: string | undefined; item: {} | null | undefined; desc: boolean | React.ReactChild | React.ReactFragment | React.ReactPortal | null | undefined; price: boolean | React.ReactChild | React.ReactFragment | React.ReactPortal | null | undefined; }, idx: React.Key) =>
          ...
          <Grid
            ...
            spacing={{ xs: 0, md: 3 }}
            columns={{ xs: 2, sm: 2, md: 12 }}>
            {filteredDrinkItems?.map((items: { type: string; image: string | undefined; item: {} | null | undefined; desc: boolean | React.ReactChild | React.ReactFragment | React.ReactPortal | null | undefined; price: boolean | React.ReactChild | React.ReactFragment | React.ReactPortal | null | undefined; }, idx: React.Key) =>
              <div className="home-item-group" key={idx}>
                <Grid item xs={4} className="home-items">
                  <img alt="" src={items.image} className="item-image" />
                </Grid>
