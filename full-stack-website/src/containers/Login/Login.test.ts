...
describe('checking if login page correctly fetches data', () => {
  ...
  test('making sure that userAuth is being correctly used', async () => {
    ...
    const response = await axios.post(`https://${process.env.REACT_APP_PORT}/userAuth`, exampleUserLoginInfo)
  ...
  test('making sure that userAuth is being correctly used when error occurs', async () => {
    await expect(axios.post(`https://${process.env.REACT_APP_PORT}/userAuth`, exampleWrongUserLoginInfo)).rejects.toThrow()
  })
